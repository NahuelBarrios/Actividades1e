...
int ll_count(LinkedList* this,int (*pFunc)(void*))
{
    ...
    if(this != NULL && pFunc != NULL)//verifico que la lista y la funcion no esten vacias
    {
        retorno = 0;

        for(i=0;i<ll_len(this);i++) // recorro el largo de la lista
        {
            retorno += pFunc(ll_get(this, i)); // pFunc devuelve 1 cuando hay que contar y lo acumula
        }
    }
    return retorno;
}
